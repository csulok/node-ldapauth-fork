 ...
 *        Optional, default all. Array of attributes to fetch from LDAP server.
 *    logger {Module}
 *        Optional. The require'd bunyan module to use for logging. If given
 *        this will result in TRACE-level logging for ldapauth.
 *    verbose {Boolean}
 *        Optional, default false. If `logger` is also given, this will add
...
function LdapAuth(opts) {
  ...
  if (opts.logger && opts.verbose) {
    this.clientOpts.log = opts.logger;
  }
